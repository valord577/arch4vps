...
if [ "${uefi_supported}" != "1" ]; then
  abort "Only machines booted with UEFI are supported."
fi
...
# Confirm the ESP
ESP="/boot/efi"
prtln "By default, the ESP at:"
prtln "  > ${ESP}"
focus "Which path is the ESP mounted to?"
read -p "  > (if empty, use default): " ESP_IN
if [ "${ESP_IN}" != "" ]; then
  ESP=${ESP_IN}
fi
debug "ESP: ${ESP}"
...
mount --bind "${ESP}" "/${arch_rootfs}/mnt${ESP}"
...
pacstrap -M /mnt base linux linux-firmware grub vim openssh efibootmgr
EOT
